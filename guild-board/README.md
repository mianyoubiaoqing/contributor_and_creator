...
默认项目状态已经不再包含示例队伍和示例任务。顶部的 **清空协作数据** 是危险操作，不是只清演示数据。点击后必须手动输入 `清空协作数据` 才会继续执行。它会清空当前项目状态 JSON 中的：
...
它会保留当前项目配置，不会删除 Supabase Auth 用户账号，也不会删除 `mvp_project_members` 中的云端成员关系。成员通过邀请码加入后，可以在 **成员与邀请** 页签点击 **同步到任务成员池**，把已批准的云端成员同步为任务负责人和复核人选项。
